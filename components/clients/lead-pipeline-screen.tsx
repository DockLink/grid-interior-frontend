...
import { getDemoPipeline } from "@/lib/clients/demo-data";
import { clientRoute } from "@/types/navigation";
import type { CreateClientPayload, LeadSource, LeadStage, PipelineCard } from "@/types/clients";
import { cn } from "@/lib/utils";

type ColumnId = LeadStage;
...
function demoPipeline(): Record<ColumnId, PipelineCard[]> {
  return getDemoPipeline();
}

function AuthCallout() {
  return (
    <div className="mb-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-[13px] text-amber-900">
      Live pipeline requires auth. Copy <code className="font-mono">.env.local.example</code> to{" "}
      <code className="font-mono">.env.local</code> and set{" "}
      <code className="font-mono">NEXT_PUBLIC_ENABLE_AUTH=true</code>. Showing demo pipeline below.
    </div>
  );
}
...
export function LeadPipelineScreen() {
  ...
  const { pipeline, isLoading, error } = useLeadPipeline();
  const { updateClient, createClient, isCreating } = useClients({ page: 1, limit: 1 }, { enabled: false });

  const [sourceFilter, setSourceFilter] = useState<LeadSource | "All">("All");
  const [draggingId, setDraggingId] = useState<string | null>(null);
  const [showAdd, setShowAdd] = useState(false);
  const [columns, setColumns] = useState<Record<ColumnId, PipelineCard[]>>(demoPipeline());

  useEffect(() => {
    if (!authDisabled && pipeline) {
      setColumns(pipeline);
    }
  }, [authDisabled, pipeline]);
  ...
  const handleDrop = (targetCol: ColumnId) => {
    ...
    if (movedCard) {
      ...
      setColumns(updated);
      if (!authDisabled) {
        void updateClient(draggingId, { stage: targetCol }).catch((err) => {
  ...
  return (
    <div className="flex min-h-full flex-col px-8 py-7">
      ...
      {!authDisabled && isLoading ? (
        <div className="flex flex-1 items-center justify-center py-16 text-sm text-[var(--figma-gray500)]">
          Loading pipeline…
        </div>
      ) : (
        <>
      ...
      {showAdd ? (
        <ClientFormModal
          mode="create"
          title="Add New Lead"
          subtitle="Create a lead and add it to the pipeline"
          defaultStatus="Lead"
          defaultStage="new"
          ...
          onSubmit={async (payload) => {
            ...
            await createClient({ ...(payload as CreateClientPayload), status: "Lead", stage: "new" });
