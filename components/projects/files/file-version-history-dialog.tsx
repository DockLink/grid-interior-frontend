  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[480px] border-[var(--figma-border)] bg-white">
        ...
        <DialogBody>
          <p className="mb-3 truncate text-[13px] font-medium text-[var(--figma-navy)]">{fileName}</p>

          {loading && (
            <p className="py-6 text-center text-sm text-[var(--figma-gray500)]">Loading…</p>
          )}

          {!loading && versions.length === 0 && (
            <p className="py-6 text-center text-sm text-[var(--figma-gray500)]">No version history found.</p>
          )}
            ...
            <div className="space-y-1.5">
              {versions.map((v, i) => (
                <div
                  ...
                  style={{
                    background: i === 0 ? "rgba(14,124,134,0.10)" : "#F9FAFB",
                  }}
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span
                        ...
                        style={{
                          background: i === 0 ? "var(--figma-teal)" : "var(--figma-gray100)",
                          color: i === 0 ? "white" : "var(--figma-gray500)",
                        }}
                      >
                        v{v.version}
                      </span>
                      {i === 0 && (
                        <span className="text-[11px] font-medium text-[var(--figma-teal)]">Current</span>
                      )}
                      {v.isSuperseded && (
                        <span className="text-[11px] text-[var(--figma-gray500)]">Superseded</span>
                      )}
                    </div>
                    <p className="mt-0.5 text-[12px] text-[var(--figma-gray500)]">
                  ...
                  <button
                    ...
                    className="ml-3 shrink-0 text-[var(--figma-gray400)] hover:text-[var(--figma-teal)]"
